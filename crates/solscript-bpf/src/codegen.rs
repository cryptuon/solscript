...
impl<'a, 'ctx> Compiler<'a, 'ctx> {
    ...
    fn declare_state_var(&mut self, var: &StateVar) -> Result<()> {
        ...
        self.state_vars
            .insert(var.name.name.to_string(), (global.as_pointer_value(), ty));
        ...
        if let solscript_ast::TypeExpr::Path(path) = &var.ty {
            ...
            if self.type_mapper.get_struct(&type_name).is_some() {
                self.state_var_struct_names
                    .insert(var.name.name.to_string(), type_name.to_string());
            }
        }
    ...
    fn declare_struct(&mut self, s: &StructDef) -> Result<()> {
        let field_names: Vec<String> = s.fields.iter().map(|f| f.name.name.to_string()).collect();
        ...
        self.type_mapper
            .register_struct(&s.name.name, &field_names, &field_types);
        Ok(())
    }
    ...
    fn compile_function(&mut self, f: &FnDef) -> Result<()> {
        ...
        let function = self.module.get_function(&fn_name).ok_or_else(|| {
            BpfError::CodegenError(format!("Function {} not declared", f.name.name))
        })?;
        ...
        for (i, param) in f.params.iter().enumerate() {
            ...
            let alloca = self
                .builder
                .build_alloca(ty, &param.name.name)
            ...
            if let solscript_ast::TypeExpr::Path(path) = &param.ty {
                ...
                if self.type_mapper.get_struct(&type_name).is_some() {
                    self.variable_struct_names
                        .insert(param.name.name.to_string(), type_name.to_string());
                }
            }
        }
        ...
        let is_view = f
            .modifiers
            .iter()
            .any(|m| m.name.name == "view" || m.name.name == "pure");
        if !is_view {
    ...
    fn compile_var_decl(&mut self, decl: &VarDeclStmt) -> Result<()> {
        ...
        let alloca = self
            .builder
            .build_alloca(ty, &decl.name.name)
            .map_err(|e| BpfError::LlvmError(e.to_string()))?;
        ...
        if let solscript_ast::TypeExpr::Path(path) = &decl.ty {
            ...
            if self.type_mapper.get_struct(&type_name).is_some() {
                self.variable_struct_names
                    .insert(decl.name.name.to_string(), type_name.to_string());
            }
        }
    ...
    fn compile_assignment(&mut self, assign: &AssignExpr) -> Result<()> {
        ...
        let final_value = match assign.op {
            ...
            AssignOp::AddAssign => {
                let current = self
                    .builder
                    .build_load(value.get_type(), ptr, "load")
                    .map_err(|e| BpfError::LlvmError(e.to_string()))?;
    ...
    fn compile_lvalue(&mut self, expr: &Expr) -> Result<PointerValue<'ctx>> {
        match expr {
            Expr::Ident(ident) => {
                ...
                Err(BpfError::CodegenError(format!(
                    "Undefined variable: {}",
                    ident.name
                )))
            }
            Expr::FieldAccess(access) => {
                ...
                if let Some(struct_name) = self.get_expr_struct_name(&access.expr) {
                    ...
                    if let Some((field_idx, _field_ty)) = self
                        .type_mapper
                        .get_field_index(&struct_name, &access.field.name)
                    {
                        // Get the struct type
    ...
    fn compile_emit(&mut self, emit: &EmitStmt) -> Result<()> {
        ...
        if let Some(log_fn) = self.intrinsics.get_sol_log(self.module) {
            ...
            let msg_global = self
                .module
                .add_global(msg_const.get_type(), None, "event_msg");
            ...
            self.builder
                .build_call(log_fn, &[msg_ptr.into(), msg_len.into()], "log")
                .map_err(|e| BpfError::LlvmError(e.to_string()))?;
        }
        Ok(())
    }
    ...
    fn compile_require(&mut self, req: &RequireStmt) -> Result<()> {
        ...
        if let Some(panic_fn) = self.intrinsics.get_sol_panic(self.module) {
            ...
            let msg_global = self
                .module
                .add_global(msg_const.get_type(), None, "panic_msg");
            msg_global.set_initializer(&msg_const);
            ...
            self.builder
                .build_call(
                    ...
                    &[
                        ...
                        self.context
                            .i64_type()
                            .const_int(msg.len() as u64, false)
                            .into(),
    ...
    fn compile_revert(&mut self, _rev: &RevertStmt) -> Result<()> {
        if let Some(panic_fn) = self.intrinsics.get_sol_panic(self.module) {
            ...
            let msg_global = self
                .module
                .add_global(msg_const.get_type(), None, "revert_msg");
            msg_global.set_initializer(&msg_const);
    ...
    fn compile_expr(&mut self, expr: &Expr) -> Result<BasicValueEnum<'ctx>> {
        match expr {
            ...
            _ => Err(BpfError::Unsupported(format!(
                "Expression type: {:?}",
                expr
            ))),
        }
    ...
    fn compile_literal(&mut self, lit: &Literal) -> Result<BasicValueEnum<'ctx>> {
        match lit {
            Literal::Int(n, _) => Ok(self.context.i64_type().const_int(*n as u64, false).into()),
            Literal::HexInt(s, _) => {
                let n = u128::from_str_radix(s.trim_start_matches("0x"), 16).unwrap_or(0);
                Ok(self.context.i64_type().const_int(n as u64, false).into())
            }
            Literal::Bool(b, _) => Ok(self.context.bool_type().const_int(*b as u64, false).into()),
            Literal::String(s, _) => {
    ...
    fn compile_field_access(&mut self, access: &FieldAccessExpr) -> Result<BasicValueEnum<'ctx>> {
        ...
        if let Some(struct_name) = self.get_expr_struct_name(&access.expr) {
            ...
            if let Some((field_idx, field_ty)) = self
                .type_mapper
                .get_field_index(&struct_name, &access.field.name)
            {
    ...
    fn compile_ternary(&mut self, ternary: &TernaryExpr) -> Result<BasicValueEnum<'ctx>> {
        ...
        let phi = self
            .builder
            .build_phi(then_val.get_type(), "ternphi")
            .map_err(|e| BpfError::LlvmError(e.to_string()))?;
    ...
    fn generate_entrypoint(&mut self, _contract: &ContractDef) -> Result<()> {
        ...
        let num_accounts = self
            .builder
            .build_load(i64_type, input_ptr, "num_accounts")
            .map_err(|e| BpfError::LlvmError(e.to_string()))?
            .into_int_value();
        ...
        let instr_len_ptr = unsafe {
            self.builder
                .build_gep(i8_type, input_ptr, &[offset_to_instr_len], "instr_len_ptr")
                .map_err(|e| BpfError::LlvmError(e.to_string()))?
        };
        ...
        let _instr_len = self
            .builder
            .build_load(i64_type, instr_len_ptr, "instr_len")
            .map_err(|e| BpfError::LlvmError(e.to_string()))?
            .into_int_value();

        // Get pointer to instruction data (8 bytes after length)
        let instr_data_offset = self
            .builder
            .build_int_add(
                offset_to_instr_len,
        ...
        let instr_data_ptr = unsafe {
            self.builder
                .build_gep(i8_type, input_ptr, &[instr_data_offset], "instr_data_ptr")
                .map_err(|e| BpfError::LlvmError(e.to_string()))?
        };
        ...
        let discriminator = self
            .builder
            .build_load(i64_type, instr_data_ptr, "discriminator")
            .map_err(|e| BpfError::LlvmError(e.to_string()))?
            .into_int_value();
        ...
        if self.compiled_functions.is_empty() {
            ...
            for func_info in &functions {
                let func_bb = self
                    .context
                    .append_basic_block(entrypoint, &format!("call_{}", func_info.name));
            ...
            // Build switch
            self.builder
                .build_switch(discriminator, unknown_bb, &cases)
                .map_err(|e| BpfError::LlvmError(e.to_string()))?;
            ...
            for (i, func_info) in functions.iter().enumerate() {
                ...
                let _call_result = self
                    .builder
                    .build_call(func_info.function, &[], &format!("call_{}", func_info.name))
                    .map_err(|e| BpfError::LlvmError(e.to_string()))?;
        ...
        self.builder
            .build_return(Some(&i64_type.const_int(1, false))) // Return error code 1
            .map_err(|e| BpfError::LlvmError(e.to_string()))?;
