...
"#, self.program_name)
    ...
    fn generate_state_types(&self, ir: &SolanaProgram) -> Result<String, CodegenError> {
        ...
        for field in &ir.state.fields {
            ...
            output.push_str(&format!("  {}: {};\n", to_camel_case_lower(&field.name), ts_type));
        }
        output.push_str("}\n\n");

        // Mapping entry types
        for mapping in &ir.mappings {
            output.push_str(&format!("export interface {}Entry {{\n", to_camel_case(&mapping.name)));
    ...
    fn generate_error_types(&self, ir: &SolanaProgram) -> Result<String, CodegenError> {
        ...
        for error in &ir.errors {
            output.push_str(&format!("  {} = '{}',\n", to_camel_case(&error.name), error.name));
        }
...
"#, mapping.name, to_camel_case(&mapping.name), to_snake_case(&mapping.name)));
        }
    ...
    fn generate_instruction_method(&self, instruction: &Instruction) -> Result<String, CodegenError> {
    ...
    fn solana_type_to_ts(&self, ty: &SolanaType) -> String {
        match ty {
            ...
            SolanaType::Mapping(key, value) => {
                format!("Map<{}, {}>", self.solana_type_to_ts(key), self.solana_type_to_ts(value))
            }
